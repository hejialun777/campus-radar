 ...
 * 校园机会雷达 · 数据层
 * ----------------------------------------------------------------------------
 * 演示数据，共 26 条（01—26），时间背景为 2026 年 9 月 16 日。
 *
 * 数据层的三条原则：
 *   1. 数据里没写的事实，一律留 null，由 UI 显示为「未注明」，绝不推测补全。
 *   2. 补充通知（09、20）不单独成为一条信息，而是挂到主信息（01、03）的 updates 上，
 *      避免同一件事在列表里出现两次、让用户自己判断哪条更新。
 *   3. 时间一律写成 ISO 字符串，交给 UI 按当前时间基准实时推算状态。
 *      （注意：数据里 02/10/18 三条活动本身就在 9 月 19 日，那是活动日期，不是时间基准。）
 *
 * 字段说明：
...
window.RAW_ITEMS = [
  ...
  /* ---------- 04：已结束，等回放 ---------- */
  {
    ...
    schedule: [
      { label: '直播', at: '2026-09-18T19:30', place: '线上' }
    ],
  ...
  /* ---------- 19：报名已截止，只能候补 ---------- */
  {
    ...
    deadlines: [
      { label: '报名截止', at: '2026-09-18T22:00' }
    ],
    standby: true,
    audience: '全校学生',
    zeroBase: true,
...
/* 演示默认的时间基准：「今天」算作 2026 年 9 月 16 日。
 * 页面首次打开就锁在这天，保证每个人看到的演示效果一致；
 * 在「我的 → 设置」里可以随时切回真实时间。 */
window.DEFAULT_TIMEBASE = '2026-09-16T09:00:00';
